...
// SDK - BUG #4 - patching shouldn't require to be done by Admin, could be done by "self"
...
router.patch(
  ...
  async function (req, res, next) {
    try {
      ...
      // SDK - BUG #5 - certain fields shouldn't be changed if non-admin, some ever
      if (!req.curr_admin) {
        if (req.body.admin) {
          throw new ExpressError(
            "Only admin can change admin status for a user.",
            401
          );
        }
      }
      if (req.body.username) {
        throw new ExpressError("Can't change username, it's primary key.", 401);
      }
